const ShayariImage = require ("../../models/Shayari/ShayariImageModel")

addShayariImage=(req,res)=>{
    var validation =""
    if(req.body.title==""){
...
getallShayariImage = (req, res) => {
    ShayariImage.find(req.body)
  ...
  const updateShayariImageStatus = async (req, res) => {
    try {
      ...
      const sher = await ShayariImage.findOne({ _id: formData._id });
