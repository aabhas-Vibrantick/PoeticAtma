const SherImage = require ("../../models/Sher/SherImageModel")

addSherImage=(req,res)=>{
    var validation =""
    if(req.body.title==""){
...
getallSherImage = (req, res) => {
    SherImage.find(req.body)
  ...
  const updateSherImageStatus = async (req, res) => {
    try {
      ...
      const sher = await SherImage.findOne({ _id: formData._id });
