...
export default function PenddingSherList() {
  ...
  useEffect(() => {
    ...
    apiServices.getallsher().then(data => {
      if (data.data.success) {
        const filteredShers = data.data.data.filter((sher) => sher.status === false);
      ...
      if (data.data.success) {
        ...
        apiServices.getallsher().then(updatedData => {
          if (updatedData.data.success) {
            const filteredShers = updatedData.data.data.filter((sher) => sher.status === false);
            setAllSher(filteredShers);
  ...
  const columns = [
    ...
    { name: 'Author', selector: row => row.userId?.name }, // Add User ID column
