...
export default function SherCategoryList() {
  ...
  useEffect(() => {
    setLoading(true);
    apiServices.getall_sher_category()
      .then(data => {
  ...
  const deleteCategory = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiServices.delete_sher_data({ _id: id })
          .then(data => {
            if (data.data.success) {
  ...
  const changeStatus = (id, status) => {
    ...
    apiServices.updateSherCategoryStatus({ _id: id, status: upstatus })
      .then(response => {
      ...
      <main className="main-container adminbody bg-light py-4">
        <div className="container-fluid col-10">
          <div className="text-center mb-4">
            <h2 className="text-dark fw-bold">Sher Category List</h2>
