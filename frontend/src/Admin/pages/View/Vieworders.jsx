...
export default function Orderlist() {
  ...
  useEffect(() => {
    ...
    apiServices.getallorder().then(data => {
  ...
  const deleteOrder = (id) => {
    ...
    apiServices.deleteOrder(data).then(data => {
      if (data.data.success) {
  ...
  const columns = [
    ...
    { name: 'Order', selector: row => row._id, sortable: true, },
    ...
    { name: 'Total', selector: 'sub_total', sortable: true, }, 
    {
      ...
      cell: row => (
        <div>
          ...
          <button onClick={() => deleteOrder(row._id)} className='btn btn-outline-dark mx-1 my-1' style={{width:"100px",height:"40px"}}>Delete</button>
          <button
