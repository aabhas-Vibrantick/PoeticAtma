...
export default function UserProseList() {
  ...
  const changeStatus = (proseId, currentStatus) => {
    setLoading(true);
    const data = { _id: proseId, status: currentStatus ? "0" : "1" };
    apiServices
      .updateProseStatus(data)
      .then((res) => {
        if (res.data.success) {
          toast.success(res.data.message);
          apiServices.getallprose({ userId: id }).then((updated) => {
            if (updated.data.success) {
              setAllProse(updated.data.data);
            }
          });
        } else {
          toast.error(res.data.message);
  ...
  const filteredProse = allProse.filter(
    (prose) =>
      ...
      prose.Category_id?.Category_name?.toLowerCase().includes(
      ...
      <main className="main-container adminbody bg-light py-4">
        <div className="container-fluid">
          <div className="text-center mb-4">
            <h2 className="text-dark fw-bold">My Prose List</h2>
